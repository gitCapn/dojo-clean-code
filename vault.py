...
def print_account(account_list):
    ...
    if type(account_list) == dict:
        table.add_row(account_list['website_name'], account_list['username'], account_list['password'])
    else:
...
def handle_show_account(p_list):
    ...
    account_choosen = None
    for i in range(len(p_list)):
        if p_list[i]["website_name"] == website_name:
            account_choosen = p_list[i]
    print_account(account_choosen)
...
def delete_account_from_list(account_list, account_name):
    ...
    for list in account_list:
        if list["website_name"] != account_name:
            new_account_list.append(list)
        else:
            continue
...
def handle_delete_accounts(password):
    save_account_list(None, password)
    return []
...
def main():
    ...
    while True:
        ...
        if option == "1":
            ...
            print_account(p_list)

        elif option == "6":
            p_list = handle_delete_accounts(master_password)
        else:
